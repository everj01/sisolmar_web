    ...
    public function getMatriculasMigraPersonal($cursoId)
    {
        try {
            ...
            $personalData = \DB::table('sw_MIGRA_PERSONAL')
                ->whereIn('CODI_PERS', $codigosPersonal)
                ->select([
                    ...
                    'CODI_CARG as cargo',
                    'SUCU_CODIGO',
                    'EMPR_CODIGO',  // código de empresa interna (PCI)
                ])
                ->get()
                ->map(function($item) {
            ...
            if (!empty($codigosSucursal)) {
                $sucData = \DB::table('sw_MIGRA_SISO_SUCURSAL')
                    ->whereIn('SUCU_CODIGO', $codigosSucursal)
                    ->select('SUCU_CODIGO', 'SUCU_ABREVIATURA', 'EMPR_CODIGO')
                    ->get();
                foreach ($sucData as $suc) {
