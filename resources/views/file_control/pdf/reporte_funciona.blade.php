...
<body>
    @foreach ($personas as $pers)
        ...
        @foreach ($imagenes as $index => $img)
            @if ($img['codPersonal'] === $pers['codPersonal'])

                @if (!in_array($img['documento'], $documentosMostrados))
                    <h4 style="text-align: center;">{{ $img['documento'] }}</h4>
                    @php
                        $documentosMostrados[] = $img['documento'];
                    @endphp

                    @php
                        $nombreDoc = strtoupper(trim($img['documento']));
                    @endphp
                    
                @endif

                <img src="{{ $img['ruta'] }}" alt="Imagen de {{ $img['documento'] }}" style="max-width: {{ $img['ancho'] }}; max-height: 1250px; display: block; margin: 0 auto; object-fit: contain; margin-bottom: 10px;">

                @php
                    $esUltimaImagenDeDocumento = true;
                    // Busca si hay otra imagen del mismo documento más adelante
                    for ($i = $index + 1; $i < count($imagenes); $i++) {
                        if (
                            $imagenes[$i]['codPersonal'] === $pers['codPersonal'] &&
                            $imagenes[$i]['documento'] === $img['documento']
                        ) {
                            $esUltimaImagenDeDocumento = false;
                            break;
                        }
                    }
                @endphp

                @if ($esUltimaImagenDeDocumento)
                    <div style="page-break-after: always;"></div>
                @endif
